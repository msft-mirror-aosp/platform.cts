...
import static com.android.tradefed.device.UserInfo.FLAG_GUEST;
...
public abstract class BaseMultiUserTest extends BaseHostJUnit4Test {
    ...
    protected final void assumeNotRoot() throws DeviceNotAvailableException {
    ...
    protected final void assumeSupportsSecondaryUser() throws DeviceNotAvailableException {
        assumeCanCreateUserOfType(USER_TYPE_FULL_SECONDARY);
    }

    protected final void assumeSupportsGuest() throws DeviceNotAvailableException {
        assumeCanCreateUserOfType(USER_TYPE_FULL_GUEST);
    }

    protected final void assumeSupportsRestrictedUser() throws DeviceNotAvailableException {
        assumeCanCreateUserOfType(USER_TYPE_FULL_RESTRICTED);
    }

    protected final int createRestrictedProfile(int userId)
            throws DeviceNotAvailableException, IllegalStateException {
        String output =
                runShellCommand(
                        "pm create-user --profileOf %d --restricted TestUser_%d",
    ...
    protected final int createGuestUser() throws Exception {
    ...
    protected final int getGuestUser() throws Exception {
    ...
    protected final void assumeGuestDoesNotExist() throws Exception {
        assumeTrue("Device already has a guest user", getGuestUser() == -1);
    }

    protected final void assumeIsAutomotive() throws Exception {
        assumeTrue("Device does not have " + FEATURE_AUTOMOTIVE,
                getDevice().hasFeature(FEATURE_AUTOMOTIVE));
    }

    protected final void assertSwitchToUser(int toUserId) throws Exception {
        final boolean switchResult = getDevice().switchUser(toUserId);
        assertWithMessage("Switched to user %s", toUserId).that(switchResult).isTrue();
        assertCurrentUser("after switch", toUserId);
    }

    protected final void assertUserNotPresent(int userId) throws Exception {
        assertWithMessage("User ID %s should not be present", userId)
                .that(getDevice().listUsers()).doesNotContain(userId);
    }

    protected final void assertUserPresent(int userId) throws Exception {
    ...
    @FormatMethod
    protected final String runShellCommand(@FormatString String fmt, @Nullable Object... args)
            throws DeviceNotAvailableException {
        String cmd = String.format(fmt, args);
        CLog.d("runShellCommand(%s): ", cmd);
    ...
    private void removeTestUsers() throws Exception {
        ...
        for (int userId : allUsers) {
            if (!mFixedUsers.contains(userId)) {
                ...
                boolean removed = getDevice().removeUser(userId);
                if (!removed) {
                    CLog.w("Failed to remove user %d", userId);
                }
            }
        }
    }

    static final class AppCrashOnBootError extends AssertionError {
    ...
    public static final class AppCrashRetryRule implements TestRule {
