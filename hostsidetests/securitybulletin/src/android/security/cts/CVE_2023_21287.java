...
public class CVE_2023_21287 extends NonRootSecurityTestCase {

    @Test
    @AsbSecurityTest(cveBugId = 278221085)
    public void testPocCVE_2023_21287() {
        try {
            final ITestDevice device = getDevice();
            final String libraryName = "libft2.so";
            final String abi = device.getProperty("ro.product.cpu.abi");
            final String libraryPath =
                    "/system/lib" + (abi.contains("x86_64") || abi.contains("arm64") ? "64" : "");

            // Get function offset.
            final List<String> functionNames = List.of("tt_apply_mvar");
            ...
            final String binaryName = "CVE-2023-21287";
            NativePoc.builder()
                    .pocName(binaryName)
                    .bitness(libraryPath.contains("lib64") ? ONLY64 : ONLY32)
                    .args(libraryPath + "/" + libraryName, functionOffset)
