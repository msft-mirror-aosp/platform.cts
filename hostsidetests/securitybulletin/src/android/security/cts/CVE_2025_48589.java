...
public class CVE_2025_48589 extends NonRootSecurityTestCase {

    @Test
    @AsbSecurityTest(cveBugId = 362492829)
    public void testPocCVE_2025_48589() {
        try {
            // Install the helper-app in primary user.
            final ITestDevice device = getDevice();
            final int primaryUserId = device.getCurrentUser();
            installPackageAsUser(
                    "CVE-2025-48589-helper.apk", true /* grant permission */, primaryUserId);

            // Start the helper-app.
            final String helperAppPackageName = "android.security.cts.CVE_2025_48589_helper";
            final String helperAppActivityName = helperAppPackageName + ".MainActivity";
            runAndCheck(
                    device,
                    String.format(
                            "am start -n %s/%s", helperAppPackageName, helperAppActivityName));
            assume().withMessage("Could not start the Helper App!!!")
                    .that(
                            poll(
                                    () -> {
                                        try {
                                            return isActivityVisible(device, helperAppActivityName);
                                        } catch (Exception e) {
                                            if (e.getMessage()
                                                    .contains("Dumpsys output does not contain")) {
            ...
            int secondaryUserId = secondaryUser.getTestUserId();
            try (AutoCloseable withSecondaryUser = secondaryUser.withUser();
                    AutoCloseable withSecondaryUserSwitched =
                            switchToSecondaryUser(primaryUserId, secondaryUserId, device)) {
                // Install test-app in secondary user.
                installPackageAsUser(
                        "CVE-2025-48589-test.apk", true /* grant permission */, secondaryUserId);

                // Run the test with secondary user on the device.
                runDeviceTests(
                        new DeviceTestRunOptions("android.security.cts.CVE_2025_48589_test")
    ...
    private AutoCloseable switchToSecondaryUser(
            int primaryUserId, int secondaryUserId, ITestDevice device) throws Exception {
