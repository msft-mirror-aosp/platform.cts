...
public class CVE_2024_49745 extends NonRootSecurityTestCase {

    @Test
    @AsbSecurityTest(cveBugId = 370831157)
    public void testPocCVE_2024_49745() {
        try {
            // Check for the device's architecture.
            final ITestDevice device = getDevice();
            final String abi = device.getProperty("ro.product.cpu.abi");
            boolean is64BitArch = abi.contains("x86_64") || abi.contains("arm64");
            final String libraryPath = "/system/lib" + (is64BitArch ? "64" : "") + "/libbinder.so";

            // Fetch the offset of the vulnerable function.
            final File libraryFile = new File(libraryPath);
            final String mangledFunctionName = "_ZN7android6Parcel8growDataEm";
            final String functionOffset =
                    getFunctionOffsetsAsCmdLineArgs(
                            new Ghidra(this), libraryFile, List.of(mangledFunctionName));

            // Run PoC to detect the vulnerability.
            NativePoc.builder()
                    .bitness(is64BitArch ? ONLY64 : ONLY32)
                    .pocName("CVE-2024-49745")
                    .args(libraryPath, functionOffset)
