...
public class CVE_2025_48555 extends NonRootSecurityTestCase {

    @AsbSecurityTest(cveBugId = 396666065)
    @Test
    public void testPocCVE_2025_48555() throws Exception {
        try {
            ...
            try (AutoCloseable asWorkUser =
                    workUser.name("CVE_2025_48555_Work_User").managed(primaryUserId).withUser()) {
                // Install helper app in work user.
                final int workUserId = workUser.getTestUserId();
                installPackageAsUser(
                        device,
                        "CVE-2025-48555-helper.apk",
                        true /* grantPermission */,
                        workUserId);

                // Enable screen lock for the work user.
                final String pin = "1234";
                try (AutoCloseable lockScreen = new LockSettingsUtil(device).withPin(pin)) {
                    ...
                    try (AutoCloseable withPocAutofillServiceAsDefault =
                            withSetting(
                                    ...
                                    "0",
                                    workUserId)) {
                        // Launch PocActivity from the helper app to switch to the work profile and
                        // generate a notification within it.
                        final String pocActivity =
                                "android.security.cts.CVE_2025_48555_helper/.PocActivity";
                        runAndCheck(device, "am start -n " + pocActivity + " --user " + workUserId);
                                        ...
                                        poll(
                                                () -> {
                                                    try {
                                                        ...
                                                        throw new IllegalStateException(throwBack);
                                                    }
                                                }))
                                .isTrue();
                        ...
                        installPackageAsUser(
                                device,
                                "CVE-2025-48555-test.apk",
                                false /* grantPermission */,
                                primaryUserId);

                        // Run DeviceTest.
                        runDeviceTests(
                                new DeviceTestRunOptions("android.security.cts.CVE_2025_48555_test")
