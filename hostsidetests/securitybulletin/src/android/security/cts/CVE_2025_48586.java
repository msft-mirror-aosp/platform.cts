...
public class CVE_2025_48586 extends NonRootSecurityTestCase {

    @Test
    @AsbSecurityTest(cveBugId = 337784859)
    public void testPocCVE_2025_48586() {
        try {
            ...
            installPackageAsUser(
                    device, "CVE-2025-48586-test.apk", false /* grantPermission */, primaryUserId);

            // Create a work profile for the primary user
            SecondaryUser workUser = new SecondaryUser(device);
            try (AutoCloseable asWorkUser =
                    workUser.name("CVE_2025_48586_work_user").managed(primaryUserId).withUser()) {
                // Install helper app and run device test of helper app in
                // 'CVE_2025_48586_work_user' to add a contact
                final int workUserId = workUser.getTestUserId();
                final String workProfileContactNumber = "cve_2025_48586";
                installPackageAsUser(
                        device,
                        "CVE-2025-48586-helper.apk",
                        true /* grantPermission */,
                        workUserId);
                runDeviceTests(
                        new DeviceTestRunOptions("android.security.cts.CVE_2025_48586_helper")
                ...
                // created in 'CVE_2025_48586_work_user'
                runDeviceTests(
                        new DeviceTestRunOptions("android.security.cts.CVE_2025_48586_test")
