...
do
    python "utils/utils_test/${M}_tests.py" 2>&1 | grep -q "OK" || \
...
echo -e "**For default camera app testing, ensure all windows appearing upon launch are cleared, and QR code scanning is disabled to prevent any interference during testing.**\n\n"
