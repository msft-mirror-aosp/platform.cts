...
def switch_default_camera(dut, facing, log_path):
  ...
  flip_camera_pattern = (
      ...
      r'front_back_switcher|switch_camera_button|'
      r'camera_switch_button|switch to rear camera|capture_bar_camera_switch)'
  )
  non_switch_pattern = (r'(flash|panorama|video|photo|portrait|supermode|mode|hdr|blur'
                        r'bg|fast_function_bar|beauty|night|more|exposure|'
                        r'quick_switcher|mode_chip_text|add|hot_area|hdr_toggle_button|function_toggle_button)')
...
def launch_and_take_capture(dut, pkg_name, camera_facing, log_path, flip_camera=True):
