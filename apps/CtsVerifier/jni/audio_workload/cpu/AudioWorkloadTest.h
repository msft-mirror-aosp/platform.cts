...
public:
    ...
    int32_t open() {
        ...
        mSynthWorkload =
                std::make_unique<SynthWorkload>((int)0.2 * mSampleRate, (int)0.3 * mSampleRate);
     ...
     */
    int32_t getBufferSizeInFrames() const { return mBufferSizeInFrames; }
     ...
     */
    static int32_t getCpuCount() { return sysconf(_SC_NPROCESSORS_CONF); }
    ...
    int32_t stop() {
        ...
        if (mStream) {
            ...
            result = mStream->waitForStateChange(oboe::StreamState::Stopping, &next,
                                                 kTimeoutInNanos);
            if (result != oboe::Result::OK) {
