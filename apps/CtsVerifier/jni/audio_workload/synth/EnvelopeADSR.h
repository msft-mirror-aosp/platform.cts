...
public:
    EnvelopeADSR() : mAttack(0.05), mDecay(0.6), mSustainLevel(0.4), mRelease(2.5) {}
...
#define MIN_DURATION (1.0 / 100000.0)
    ...
    void setGate(bool gate) { triggered = gate; }

    bool isIdle() { return mState == State::IDLE; }
     ...
     */
    void setDecayTime(synth_float_t time) { mDecay = time; }

    synth_float_t getDecayTime() { return mDecay; }

    /**
     * Time in seconds for the rising stage of the envelope to go from 0.0 to 1.0. The attack is a
     * linear ramp.
     */
    void setAttackTime(synth_float_t time) { mAttack = time; }

    synth_float_t getAttackTime() { return mAttack; }
...
private:
    ...
    void startSustain() { mState = State::SUSTAINING; }
