...
public:
    DifferentiatedParabola() : mZ1(0), mZ2(0) {}

    virtual ~DifferentiatedParabola() = default;

    synth_float_t next(synth_float_t phase, synth_float_t phaseIncrement) {
        synth_float_t dpw;
        synth_float_t positivePhaseIncrement =
                (phaseIncrement < 0.0) ? phaseIncrement : 0.0 - phaseIncrement;
        ...
        if (positivePhaseIncrement < kDPWVeryLowFrequency) {
            ...
            // Calculate scaling
            dpw = diffed * 0.25f / positivePhaseIncrement;
        }
        return dpw;
    }
