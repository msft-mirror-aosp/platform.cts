...
JNIEXPORT jlong JNICALL Java_com_android_cts_verifier_audio_NativeAnalyzerThread_openAudio
  (JNIEnv * /*env */, jobject /* obj */, jint inputDeviceId, jint outputDeviceId, jint format) {
    ...
    aaudio_result_t result = analyzer->openAudio(inputDeviceId, outputDeviceId, format);
    if (result != AAUDIO_OK) {
        delete analyzer;
        analyzer = nullptr;
    }
