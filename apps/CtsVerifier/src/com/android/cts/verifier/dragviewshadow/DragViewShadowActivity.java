...
public class DragViewShadowActivity extends PassFailButtons.Activity {
                            ...
                            public boolean onDrag(View v, DragEvent event) {
                                ...
                                switch (action) {
                                    ...
                                    case DragEvent.ACTION_DRAG_ENDED -> {
                                        Log.d(TAG, "Global: ACTION_DRAG_ENDED. Result: " + event.getResult());
                                        // Reposition the draggable container to the last known
                                        // drag location
                                        mDragContainer.setX(mCurrentDragX - (mDragContainer.getWidth() / 2f));
                                        mDragContainer.setY(mCurrentDragY - (mDragContainer.getHeight() / 2f));
                                        ...
                                        if (mCurrentDragState == DragState.PARTIAL_OVERLAP) {
                                            Toast.makeText(
                                                    ...
                                                    .show();
                                            mCurrentDragState = DragState.NO_OVERLAP;
                                        } else if (mCurrentDragState == DragState.SUFFICIENT_OVERLAP) {
                                    ...
                                    case DragEvent.ACTION_DRAG_ENTERED, DragEvent.ACTION_DRAG_EXITED, DragEvent.ACTION_DROP -> {
                                        return true;
                                    }
                                    default -> {
                                    }
    ...
    private void checkShadowOverlap(float dropX, float dropY) {
        ...
        Rect draggableShadowRect =
                new Rect(
                        (int) (absoluteShadowCenterX - (mDragContainer.getWidth() / 2f)),
                        (int) (absoluteShadowCenterY - (mDragContainer.getHeight() / 2f)),
                        (int) (absoluteShadowCenterX + (mDragContainer.getWidth() / 2f)),
                        (int) (absoluteShadowCenterY + (mDragContainer.getHeight() / 2f)));
