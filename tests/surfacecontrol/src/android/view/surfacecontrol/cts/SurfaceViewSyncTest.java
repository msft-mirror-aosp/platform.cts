        ...
        public View createView(Context context) {
            ...
            surfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
                ...
                public void surfaceChanged(SurfaceHolder holder, int format, int width,
                    ...
                    // CountDownLatch is being used only when running with
                    // replaceContentOnDisplayWithMirror, since waitForReady returns right away
                    // otherwise.
                    if (CapturedActivity.wmCanReplaceContentOnDisplay()) {
    ...
    private final ViewFactory mVideoViewFactory = new ViewFactory() {
        ...
        public View createView(Context context) {
            ...
            surfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
                ...
                public void surfaceCreated(SurfaceHolder holder) {
                    if (CapturedActivity.wmCanReplaceContentOnDisplay()) {
                        // When using MediaPlayer, we need to wait until the first frame is drawn
                        // since it can be rendered asynchronously. Merge a commit callback
                        // transaction so the next draw into the SV gets the callback invoked when
                        // content is rendered on screen. This is a good signal to know when the
                        // test can start.
