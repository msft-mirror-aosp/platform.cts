...
public class SurfacePixelValidator2 {
    ...
    private int mRequiredNumFrames;
            ...
            if (CapturedActivity.wmCanReplaceContentOnDisplay()) {
                ...
                synchronized (mResultLock) {
                    ...
                    if (success) {
                        ...
                        Log.d(TAG,
                                "Failure (" + mPixelChecker.getLastError() + ") occurred on frame "
                                        + totalFramesSeen);
    ...
    public SurfacePixelValidator2(Point size, @Nullable Rect boundsToCheck,
        ...
        this(null, size, boundsToCheck, pixelChecker);
        mRequiredNumFrames = requiredNumFrames;
    }

    public SurfacePixelValidator2(@Nullable Context context, Point size,
            @Nullable Rect boundsToCheck, PixelChecker pixelChecker) {
        mWidth = size.x;
        mHeight = size.y;
    ...
    public void finish(CapturedActivity.TestResult testResult) {
        if (CapturedActivity.wmCanReplaceContentOnDisplay()) {
            ...
            // Post the imageReader close on the same thread it's processing data to avoid shutting
            // down while still in the middle of processing an image.
            mHandler.post(() -> {
            ...
            synchronized (mResultLock) {
                // could in theory miss results still processing, but only if latency is extremely
                // high. Caller should only call this
                testResult.failFrames = mResultFailureFrames;
                testResult.passFrames = mResultSuccessFrames;
