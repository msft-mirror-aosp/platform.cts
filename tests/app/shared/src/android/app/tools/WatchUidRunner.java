...
public class WatchUidRunner {
    ...
    static final String[] COMMAND_TO_STRING =
            new String[] {
    ...
    public WatchUidRunner(
            Instrumentation instrumentation, int uid, long defaultWaitTime, int capabilityMask) {
        ...
        ParcelFileDescriptor[] pfds =
                instrumentation
                        .getUiAutomation()
                        .executeShellCommandRw("am watch-uids --oom " + uid + maskString);
        mReadFd = pfds[0];
    ...
    public void waitFor(int cmd, String procState) {
        final WatchUidPredicate predicate =
                new WatchUidPredicate.Builder(cmd).setExpectedProcState(procState).build();
        waitFor(predicate, null);
    }

    public void waitFor(int cmd, String procState, Integer capability) {
        final WatchUidPredicate predicate =
                new WatchUidPredicate.Builder(cmd)
     ...
     */
    public void waitFor(
            @NonNull WatchUidPredicate expectedPredicate,
    ...
    public void waitFor(
        ...
        while (true) {
            ...
            if (expectedPredicate.test(line)) {
                ...
                String msg =
                        "Unexpected line hit: uid="
                                + mUidStr
                                + " cmd="
                                + COMMAND_TO_STRING[failurePredicate.cmd]
                                + " procState="
                                + failurePredicate.procState
                                + " capability="
                                + failurePredicate.capability
                                + " (Expected:"
                                + " cmd="
                                + COMMAND_TO_STRING[cmd]
                                + " procState="
        ...
        synchronized (mPendingLines) {
            while (true) {
                while (mPendingLines.size() == 0) {
                    ...
                    if (now >= waitUntil) {
                        String msg =
                                "Timed out waiting for next line: uid="
                                        + mUidStr
                                        + " cmd="
                                        + COMMAND_TO_STRING[cmd]
                                        + " procState="
                ...
                if (cmd == CMD_PROCSTATE) {
                    if (procState != null && capability != null) {
                        if (procState.equals(line[PROCSTATE_INDEX])
                                && capability.toString().equals(line[CAPABILITY_INDEX])) {
                            return true;
                        }
