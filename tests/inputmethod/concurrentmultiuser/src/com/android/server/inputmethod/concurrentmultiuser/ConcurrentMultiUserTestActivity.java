...
public final class ConcurrentMultiUserTestActivity extends ConcurrentUserActivityBase {
    ...
    void showMyImeAndWait() {
        runOnUiThread(
                () -> {
                    // View#requestFocus() and WindowInsetsControllerCompat#show() must run on UI
                    // thread.
                    if (!mEditor.requestFocus()) {
                        Log.e(TAG, "Failed to focus on mEditor");
                        return;
                    }
                    // Compared to mImm.showSoftInput(), the call below is the recommended way to
                    // show the
                    // keyboard because it is guaranteed to be scheduled after the window is
                    // focused.
        ...
        PollingCheck.waitFor(
                ...
                () -> isMyImeVisible(),
                String.format(
                        "%s: My IME (user %d) didn't show up",
                        TAG, Process.myUserHandle().getIdentifier()));
    }
    ...
    void hideMyImeAndWait() {
        ...
        PollingCheck.waitFor(
                ...
                () -> !isMyImeVisible(),
                String.format(
                        "%s: My IME (user %d) is still shown",
                        TAG, Process.myUserHandle().getIdentifier()));
    }
}
