 ...
 * the one in AppSearch side. This won't work in the framework, since even if we close the {@link
 * ParcelFileDescriptor} in the server side, the one in the client side could still work.
 *
...
public class JetpackRevocableFileDescriptorStore implements RevocableFileDescriptorStore {
    ...
    private void setCloseListenerToFd(
        ...
        revocableFileDescriptor.setCloseListener(
                e -> {
                    synchronized (mLock) {
    ...
    /**
     * A custom {@link ParcelFileDescriptor} that provides an additional mechanism to register a
     * {@link ParcelFileDescriptor.OnCloseListener} which will be invoked when the file descriptor
     * is closed.
     *
    ...
    static class JetpackRevocableFileDescriptor extends ParcelFileDescriptor {
        ...
        /**
         * Create a new ParcelFileDescriptor wrapped around another descriptor. By default all
         * method calls are delegated to the wrapped descriptor.
         */
        JetpackRevocableFileDescriptor(@NonNull ParcelFileDescriptor parcelFileDescriptor) {
            super(parcelFileDescriptor);
        }

        void setCloseListener(@NonNull ParcelFileDescriptor.OnCloseListener onCloseListener) {
            if (mOnCloseListener != null) {
                throw new IllegalStateException("The close listener has already been set.");
            }
            mOnCloseListener = Objects.requireNonNull(onCloseListener);
        }
