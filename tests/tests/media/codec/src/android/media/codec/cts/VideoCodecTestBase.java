 ...
 * the decoder to verify frames are decodable and to calculate PSNR values for various bitrates.
 */
...
public class VideoCodecTestBase {
    ...
    // Bitrate layering constants
    protected static final String BITRATE_LAYERING_2_LAYERS = "0.5";
    protected static final String BITRATE_LAYERING_3_LAYERS = "0.4;0.6";
    protected static final String BITRATE_LAYERING_2_LAYERS_DYNAMIC = "0.7";
    protected static final String BITRATE_LAYERING_3_LAYERS_DYNAMIC = "0.6;0.8";

    protected static final double[] BITRATE_RATIOS_2_LAYERS = {0.5, 1.0};
    protected static final double[] BITRATE_RATIOS_3_LAYERS = {0.4, 0.6, 1.0};
    // Average of default and dynamic bitrate ratios
    protected static final double[] BITRATE_RATIOS_2_LAYERS_DYNAMIC = {0.7, 1.0};
    protected static final double[] BITRATE_RATIOS_3_LAYERS_DYNAMIC = {0.6, 0.8, 1.0};
     ...
     *
     * <p>Iterates through the list of available codecs and tries to find Video codec, which can
     * support either YUV420 planar or NV12 color formats.
     *
    ...
    private CodecProperties getVideoCodecProperties(boolean isEncoder, MediaFormat format)
                ...
                for (int supportedColorFormat : mSupportedColorList) {
                    for (int codecColorFormat : capabilities.colorFormats) {
                        if (codecColorFormat == supportedColorFormat) {
                            codecProperties =
                                    new CodecProperties(codecInfo.getName(), codecColorFormat);
                            Log.v(
                                    TAG,
                                    "Found target codec "
                                            + codecProperties.codecName
                                            + ". Color: 0x"
        ...
        if (codecProperties == null) {
            Log.i(
                    TAG,
                    "no suitable " + (isEncoder ? "encoder " : "decoder ") + "found for " + format);
        }
        return codecProperties;
    }
    ...
    private CodecProperties getEncoderProperties(String codecName, MediaFormat format)
            throws Exception {
        assumeTrue(
                "Media format " + format + " is not supported by " + codecName,
        ...
        assumeTrue(
                "Codec " + codecName + " doesn't support color YUV 420 color formats",
    ...
    protected class EncoderOutputStreamParameters {
        ...
        // Layering schema explicitly requesting the layer structure
        public String layeringSchema;
        // String describing the bitrate layering to be used for dynamic bitrate layering testing,
        // which triggers a change in bitrate allocation mid-stream.
        public String dynamicBitrateLayeringRatios;
        // Frame index at which the dynamic bitrate layering change should be triggered.
        public int dynamicBitrateUpdateFrameIndex;
        // String describing the bitrate layering ratios.
        public String bitrateLayeringRatios;
    ...
    protected class VideoEncodeOutput {
        ...
        public ArrayList<Integer> temporalLayerIds;

        VideoEncodeOutput(
                ArrayList<MediaCodec.BufferInfo> bufferInfo, EncodingStatisticsInfo encStat) {
            this(bufferInfo, encStat, null);
        }

        VideoEncodeOutput(
                ArrayList<MediaCodec.BufferInfo> bufferInfo,
                EncodingStatisticsInfo encStat,
                ArrayList<Integer> temporalLayerIds) {
            this.bufferInfo = bufferInfo;
            this.encStat = encStat;
            this.temporalLayerIds = temporalLayerIds;
        }
    }
    ...
    /**
     * Generates an array of default parameters for encoder output stream based on upscaling value.
     */
    protected ArrayList<EncoderOutputStreamParameters> getDefaultEncodingParameterList(
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            params.scaledYuvFilename =
                    SDCARD_DIR + File.separator + outputIvfBaseName + resolutionScales[i] + ".yuv";
    ...
    /**
     * Converts (interleaves) YUV420 planar to NV12. Assumes packed, macroblock-aligned frame with
     * no cropping (visible/coded row length == stride).
     */
    ...
    /**
     * Converts (de-interleaves) NV12 to YUV420 planar. Stride may be greater than width, slice
     * height may be greater than height.
     */
    private static byte[] NV12ToYUV420(
            int width, int height, int stride, int sliceHeight, byte[] nv12) {
    ...
    /**
     * Packs YUV420 frame by moving it to a smaller size buffer with stride and slice height equal
     * to the crop window.
     */
    private static byte[] PackYUV420(
            int left, int top, int width, int height, int stride, int sliceHeight, byte[] src) {
        ...
        for (int i = 0; i < height / 2; i++) {
            System.arraycopy(
                    src,
                    u_src_offset + (i + top) * (stride / 2) + left,
                    dst,
                    u_dst_offset + i * (width / 2),
                    width / 2);
            System.arraycopy(
                    src,
                    v_src_offset + (i + top) * (stride / 2) + left,
                    dst,
                    v_dst_offset + i * (width / 2),
                    width / 2);
        }
        return dst;
    }

    private static void imageUpscale1To2(
            byte[] src,
            int srcByteOffset,
            int srcStride,
            byte[] dst,
    ...
    /**
     * Up-scale image. Scale factor is defined by source and destination width ratio. Only 1:2 and
     * 1:4 up-scaling is supported for now. For 640x480 -> 1280x720 conversion only top 640x360 part
     * of the original image is scaled.
     */
    private static byte[] imageScale(
            byte[] src, int srcWidth, int srcHeight, int dstWidth, int dstHeight) throws Exception {
        ...
        if (dstWidth == 2 * srcWidth && dstHeight <= 2 * srcHeight) {
            ...
            imageUpscale1To2(src, 0, srcWidth, dst, 0, dstWidth, dstHeight); // Y
            imageUpscale1To2(
                    src, srcYSize, srcWidth / 2, dst, dstYSize, dstWidth / 2, dstHeight / 2); // U
            imageUpscale1To2(
                    src,
                    srcYSize * 5 / 4,
                    srcWidth / 2,
     ...
     *
     * <p>The most basic confirmation we can get about a frame being properly encoded is trying to
     * decode it. (Especially in realtime mode encode output is non- deterministic, therefore a more
     * thorough check like md5 sum comparison wouldn't work.)
     *
     * <p>Indeed, MediaCodec will raise an IllegalStateException whenever video decoder fails to
     * decode a frame, and this test uses that fact to verify the bitstream.
     *
    ...
    protected ArrayList<MediaCodec.BufferInfo> decode(
        ...
        // Create decoder.
        MediaFormat format =
                MediaFormat.createVideoFormat(codecMimeType, ivf.getWidth(), ivf.getHeight());
        ...
        while (!sawOutputEOS) {
            ...
            while (result == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED
                ...
                if (result == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    ...
                    Log.d(
                            TAG,
                            "Decoder output format change. Color: 0x"
                                    + Integer.toHexString(frameColorFormat));
            ...
            if (result >= 0) {
                ...
                Log.v(
                        ...
                        "Writing buffer # "
                                ...
                                + bufferInfo.size
                                + ". InTime: "
                                + (inPresentationTimeUs + 500) / 1000
                                + ". OutTime: "
                                + (outPresentationTimeUs + 500) / 1000);
                ...
                if (bufferInfo.size > 0) {
                    ...
                    bufferInfoCopy.set(
                            (int) presentationTimeUsDelta,
                            bufferInfo.size,
                            outPresentationTimeUs,
                            bufferInfo.flags);
    ...
    /**
     * Helper function to return InputStream from either fully specified filename (if set) or
     * resource name within test assets (if filename is not set).
     */
    ...
    protected class SvcTemporalLayerIndexValidator {
        private int mNumTemporalLayers = 0;
        private int mFrameIndex = 0; // Frame index from keyframe.
        private int mTemporalLayerCycle = 4;
        // This temporal layer index structure comes from
        // https://www.w3.org/TR/webrtc-svc/#dependencydiagrams*
        private int[][] mExpectedTemporalId = {
            {0, 1, 0, 1},
            {0, 2, 1, 2}
        };

        SvcTemporalLayerIndexValidator(int numTemporalLayers) {
            mNumTemporalLayers = numTemporalLayers;
            mFrameIndex = 0;
            assertTrue(mNumTemporalLayers > 1);
        }

        public void validate(int flags, int temporalLayerIndex) {
            if ((flags & MediaCodec.BUFFER_FLAG_SYNC_FRAME) != 0) {
                mFrameIndex = 0;
            }

            if (temporalLayerIndex < 0 || temporalLayerIndex >= mNumTemporalLayers) {
                throw new RuntimeException(
                        "Temporal Layer index is out of range: " + temporalLayerIndex);
            }
            int expectedTemporalLayerIndex =
                    mExpectedTemporalId[mNumTemporalLayers - 2][mFrameIndex % mTemporalLayerCycle];
            if (temporalLayerIndex != expectedTemporalLayerIndex) {
                throw new RuntimeException(
                        "Invalid temporal layer index: "
                                + temporalLayerIndex
                                + " expected: "
                                + expectedTemporalLayerIndex
                                + ", frame index: "
                                + mFrameIndex);
            }
            mFrameIndex++;
    ...
    protected void setTemporalLayersFormat(
            MediaFormat format, String layeringSchema, String bitrateLayering) {
        format.setString(MediaFormat.KEY_TEMPORAL_LAYERING, layeringSchema);
        format.setString(MediaFormat.KEY_VIDEO_BITRATE_LAYERING, bitrateLayering);
        // This is needed to let MediaCodec fill temporal layer id in output MediaFormat.
        format.setInteger(MediaFormat.KEY_TEMPORAL_LAYER_ID, 0);
    }
    ...
    protected class MediaEncoderAsyncHelper {
        ...
        private final ArrayList<Integer> mTemporalLayerIds;
        ...
        MediaEncoderAsyncHelper(
            ...
            mTemporalLayerIds = new ArrayList<Integer>();

            int srcFrameSize = streamParams.frameWidth * streamParams.frameHeight * 3 / 2;
            mSrcFrame = new byte[srcFrameSize];

            mYuvStream =
                    OpenFileOrResource(streamParams.inputYuvFilename, streamParams.inputResource);
        }
        ...
        public byte[] getInputFrame() {
            ...
            if (mProperties.colorFormat != CodecCapabilities.COLOR_FormatYUV420Planar) {
                return YUV420ToNV(mStreamParams.frameWidth, mStreamParams.frameHeight, mSrcFrame);
            } else {
                return mSrcFrame;
            }
        }
        ...
        public boolean saveOutputFrame(MediaEncoderOutput out) {
            if (out.outputGenerated) {
                ...
                if (out.buffer.length > 0) {
                    ...
                    // Update statistics - store presentation time delay in offset
                    long presentationTimeUsDelta =
                            out.inPresentationTimeUs - out.outPresentationTimeUs;
                    MediaCodec.BufferInfo bufferInfoCopy = new MediaCodec.BufferInfo();
                    bufferInfoCopy.set(
                            (int) presentationTimeUsDelta,
                            out.buffer.length,
                    ...
                    if (out.temporalLayerId >= 0) {
                        mTemporalLayerIds.add(out.temporalLayerId);
                    }
                }
                // Detect output EOS
        ...
        public EncoderOutputStreamParameters getStreamParams() {
        ...
        public ArrayList<Integer> getTemporalLayerIds() {
            return mTemporalLayerIds;
        }
    }

    /**
     * Video encoder wrapper class. Allows to run the encoder either in a callee's thread or in a
     * looper thread using buffer dequeue ready notification callbacks.
     *
     * <p>Function feedInput() is used to send raw video frame to the encoder input. When encoder is
     * configured to run in async mode the function will run in a looper thread. Encoded frame can
     * be retrieved by calling getOutput() function.
     */
    protected class MediaEncoderAsync extends Thread {
        ...
        private SvcTemporalLayerIndexValidator mWebRtcTemporalLayerIndexValidator;
        private String mLayeringSchema;
                    ...
                    public void onOutputBufferAvailable(
                        ...
                        logStr +=
                                ". InTime: "
                                        + (mInPresentationTimeUs + 500) / 1000
                                        + ". OutTime: "
                                        + (mOutPresentationTimeUs + 500) / 1000;
                        Log.v(TAG, logStr);
                        ...
                        if (info.size > 0 && mWebRtcTemporalLayerIndexValidator != null) {
                            if (!format.containsKey(MediaFormat.KEY_TEMPORAL_LAYER_ID)) {
                                throw new RuntimeException("Temporal layer id doesn't exist");
                            }
                            // The generic Android SVC technically allows any ordering of temporal
                            // layers as long as it can decode properly. We only enforce the strict
                            // ordering for the webrtc schema.
                            if (mLayeringSchema.contains("webrtc")) {
                                mWebRtcTemporalLayerIndexValidator.validate(
                                        info.flags,
                                        format.getInteger(MediaFormat.KEY_TEMPORAL_LAYER_ID));
                            }
                        }

                        if (info.size > 0
                                && format.containsKey(MediaFormat.KEY_TEMPORAL_LAYER_ID)) {
                            out.temporalLayerId =
                                    format.getInteger(MediaFormat.KEY_TEMPORAL_LAYER_ID);
                        } else {
                            out.temporalLayerId = -1;
                        }
        ...
        private void createCodecInternal(
                final String name, final MediaFormat format, final long timeout, boolean useNdk)
                throws Exception {
            ...
            mWebRtcTemporalLayerIndexValidator = null;
            if (useNdk) {
            ...
            if (format.containsKey(MediaFormat.KEY_TEMPORAL_LAYERING)
                    && format.containsKey(MediaFormat.KEY_TEMPORAL_LAYER_ID)) {
                String layerStr = format.getString(MediaFormat.KEY_TEMPORAL_LAYERING);
                String numTemporalLayersStr;
                if (layerStr.contains("android.generic")) {
                    int plusIndex = layerStr.indexOf("+");
                    if (plusIndex != -1) {
                        numTemporalLayersStr =
                                layerStr.substring(layerStr.lastIndexOf(".") + 1, plusIndex);
                    } else {
                        numTemporalLayersStr = layerStr.substring(layerStr.lastIndexOf(".") + 1);
                    }
                } else if (layerStr.contains("webrtc")) {
                    numTemporalLayersStr = layerStr.substring(layerStr.lastIndexOf("t") + 1);
                } else {
                    throw new RuntimeException("Unsupported temporal layering schema: " + layerStr);
                }
                int numTemporalLayers = Integer.parseInt(numTemporalLayersStr);
                mWebRtcTemporalLayerIndexValidator =
                        new SvcTemporalLayerIndexValidator(numTemporalLayers);
                mLayeringSchema = layerStr;
            }
            mCodec.start();
        ...
        private void getOutputInternal() {
            ...
            if (result >= 0) {
                ...
                if (mBufferInfo.size > 0) {
                    mOutputFrameIndex++;
                    mOutput.outPresentationTimeUs = mOutPresentationTimeUs;
                    MediaFormat format = mCodec.getOutputFormat(outputBufIndex);
                    if (mWebRtcTemporalLayerIndexValidator != null) {
                        if (!format.containsKey(MediaFormat.KEY_TEMPORAL_LAYER_ID)) {
                            throw new RuntimeException(
                                    "Temporal layer id doesn't exist: format=" + format.toString());
                        }
                        // The generic Android SVC technically allows any ordering of temporal
                        // layers as long as it can decode properly. We only enforce the strict
                        // ordering for the webrtc schema.
                    ...
                    if (format.containsKey(MediaFormat.KEY_TEMPORAL_LAYER_ID)) {
                        mOutput.temporalLayerId =
                                format.getInteger(MediaFormat.KEY_TEMPORAL_LAYER_ID);
                    } else {
                        mOutput.temporalLayerId = -1;
                    }
                }
        ...
        public void updateBitrateLayering(String bitrateLayering) throws Exception {
            ...
            runCallable(
                    new Callable<Void>() {
                        @Override
                        public Void call() throws Exception {
                            mCodec.setParameters(bitrateLayeringUpdate);
                            return null;
                        }
                    });
        }
     ...
     */
    protected VideoEncodeOutput encode(EncoderOutputStreamParameters streamParams)
            throws Exception {
        return encode(streamParams, new ArrayList<ByteBuffer>());
    }

    /**
     * Video encoding loop supporting encoding single streams with an option to run in a looper
     * thread and use buffer ready notification callbacks.
     *
     * <p>Output stream is described by encodingParams parameters.
     *
     * <p>MediaCodec will raise an IllegalStateException whenever video encoder fails to encode a
     * frame.
     *
     * <p>Color format of input file should be YUV420, and frameWidth, frameHeight should be
     * supplied correctly as raw input file doesn't include any header data.
     *
     * @param streamParams Structure with encoder parameters
     * @param codecConfigs List to be filled with codec config buffers
     * @return Returns VideoEncodeOutput, which consists of array of encoded frames information for
     *     each frame and Encoding Statistics Information.
     */
    protected VideoEncodeOutput encode(
            EncoderOutputStreamParameters streamParams, ArrayList<ByteBuffer> codecConfigs)
            throws Exception {

        ArrayList<MediaCodec.BufferInfo> bufferInfos = new ArrayList<MediaCodec.BufferInfo>();
        ArrayList<Integer> temporalLayerIds = new ArrayList<>();
        EncodingStatisticsInfo encStatInfo = new EncodingStatisticsInfo();
        Log.d(
                TAG,
                "Source resolution: " + streamParams.frameWidth + " x " + streamParams.frameHeight);
        int bitrate = streamParams.bitrateSet[0];

        // Create minimal media format signifying desired output.
        MediaFormat format =
                MediaFormat.createVideoFormat(
                        streamParams.codecMimeType,
                        streamParams.frameWidth,
                        streamParams.frameHeight);
        format.setInteger(MediaFormat.KEY_BIT_RATE, bitrate);
        CodecProperties properties = getEncoderProperties(streamParams.codecName, format);

        // Open input/output
        InputStream yuvStream =
                OpenFileOrResource(streamParams.inputYuvFilename, streamParams.inputResource);
        IvfWriter ivf =
                new IvfWriter(
        ...
        if (streamParams.temporalLayers > 1) {
            setTemporalLayersFormat(
                    format, streamParams.layeringSchema, streamParams.bitrateLayeringRatios);
        }
        format.setInteger(MediaFormat.KEY_COLOR_FORMAT, properties.colorFormat);
        format.setInteger(MediaFormat.KEY_FRAME_RATE, streamParams.frameRate);
        int syncFrameInterval =
                (streamParams.syncFrameInterval + streamParams.frameRate / 2)
        ...
        Log.d(
                ...
                "Creating encoder "
                        ...
                        + bitrate
                        + " Bitrate type: "
                        + streamParams.bitrateType
                        + ". Fps:"
                        + streamParams.frameRate
                        + ". TS Layers: "
                        + streamParams.temporalLayers
                        + ". Key frame:"
                        + syncFrameInterval * streamParams.frameRate
                        + ". Force keyFrame: "
        ...
        while (!sawOutputEOS) {
                ...
                if (inputConsumed && !sawInputEOS) {
                    ...
                    if (!sawInputEOS && bytesRead == -1) {
                        if (streamParams.frameCount == 0) {
                            ...
                            yuvStream =
                                    OpenFileOrResource(
                                            streamParams.inputYuvFilename,
                                            streamParams.inputResource);
                    ...
                    if (!sawInputEOS
                            && streamParams.dynamicBitrateUpdateFrameIndex > 0
                            && streamParams.dynamicBitrateUpdateFrameIndex == inputFrameIndex) {
                        codec.updateBitrateLayering(streamParams.dynamicBitrateLayeringRatios);
                    }
            ...
            if (out.outputGenerated) {
                ...
                if (out.buffer.length > 0) {
                    ...
                    bufferInfos.add(bufferInfoCopy);
                    if (out.temporalLayerId >= 0) {
                        temporalLayerIds.add(out.temporalLayerId);
                    }
                }
            }
        ...
        return new VideoEncodeOutput(bufferInfos, encStatInfo, temporalLayerIds);
    }
    ...
    protected VideoEncodeOutput encodeAsync(
        ...
        MediaEncoderAsync codec = new MediaEncoderAsync();
        MediaEncoderAsyncHelper helper =
                new MediaEncoderAsyncHelper(
        ...
        return new VideoEncodeOutput(bufferInfos, encStatInfo, helper.getTemporalLayerIds());
    }

    /**
     * Video encoding loop supporting encoding multiple streams at a time. Each output stream is
     * described by encodingParams parameters allowing simultaneous encoding of various resolutions,
     * bitrates with an option to control key frame and dynamic bitrate for each output stream
     * indepandently.
     *
     * <p>MediaCodec will raise an IllegalStateException whenever video encoder fails to encode a
     * frame.
     ...
     * @param codecConfigs List to be filled with codec config buffers
     * @return Returns 2D array of encoded frames information for each stream and for each frame.
     */
    protected ArrayList<ArrayList<MediaCodec.BufferInfo>> encodeSimulcast(
            ...
            ArrayList<ArrayList<ByteBuffer>> codecConfigs)
            throws Exception {
        ...
        for (EncoderOutputStreamParameters params : encodingParams) {
            int i = numEncoders;
            Log.d(TAG, "Source resolution: " + params.frameWidth + " x " + params.frameHeight);
            int bitrate = params.bitrateSet[0];

            // Create minimal media format signifying desired output.
            format[i] =
                    MediaFormat.createVideoFormat(
                            params.codecMimeType, params.frameWidth, params.frameHeight);
            ...
            if (params.temporalLayers > 0) {
                format[i].setInteger("ts-layers", params.temporalLayers);
            }
            ...
            int syncFrameInterval =
                    (params.syncFrameInterval + params.frameRate / 2) / params.frameRate; // in sec
            ...
            Log.d(
                    ...
                    "Creating encoder #"
                            ...
                            + ". Key frame:"
                            + syncFrameInterval * params.frameRate
                            + ". Force keyFrame: "
            ...
            codec[i].createCodec(
                    ...
                    properties.codecName,
                    format[i],
                    params.timeoutDequeue,
                    params.runInLooperThread,
                    params.useNdk);
                ...
                if (inputConsumed[i] && !sawInputEOS[i]) {
                    ...
                    if (params.frameCount > 0 && inputFrameIndex[i] >= params.frameCount) {
                        ...
                        Log.d(
                                ...
                                "---Enc"
                                        + i
                                        + ". Sending EOS empty frame for frame # "
                                        + inputFrameIndex[i]);
                    }
    ...
    protected class VideoEncodingStatistics {
        ...
        public ArrayList<Integer>
                mBitrates; // Bitrate values for each second of the encoded stream.
    ...
    /**
     * Calculates average bitrate and key frame interval for the encoded streams. Output mBitrates
     * field will contain bitrate values for every second of the encoded stream. Average stream
     * bitrate will be stored in mAverageBitrate field. mKeyFrames array will contain the position
     * of key frames in the encoded stream and mKeyFrameInterval - average key frame interval.
     */
    protected VideoEncodingStatistics computeEncodingStatistics(
            int encoderId, ArrayList<MediaCodec.BufferInfo> bufferInfos) {
    ...
    /**
     * Calculates maximum latency for encoder/decoder based on buffer info array generated either by
     * encoder or decoder.
     */
    ...
    /**
     * Calculates average and minimum PSNR values between set of reference and decoded video frames.
    ...
    protected VideoDecodingStatistics computeDecodingStatistics(
        ...
        InputStream referenceStream = OpenFileOrResource(referenceYuvFilename, referenceYuvRaw);
        ...
        while (true) {
            ...
            if (bytesReadRef == -1) {
                ...
                referenceStream = OpenFileOrResource(referenceYuvFilename, referenceYuvRaw);
        ...
        logStr =
                String.format(
                        ...
                        minimumUPSNR,
                        minimumVPSNR,
                        statistics.mMinimumPSNR,
                        minimumPSNRFrameIndex);
        Log.d(TAG, logStr);
    ...
    protected ArrayList<Pair<String, String>> validateTemporalLayerSubstreams(
            String inputIvfFilename,
            ...
            ArrayList<Integer> temporalLayerIds)
            throws Exception {
        IvfReader reader = new IvfReader(inputIvfFilename);
        int frameCount = reader.getFrameCount();
        if (frameCount != bufferInfos.size()) {
            throw new RuntimeException(
                    "Frame count mismatch: IVF="
                            + frameCount
                            + ", BufferInfos="
                            + bufferInfos.size());
        }

        ArrayList<Pair<String, String>> outputFilenames = new ArrayList<>();
        IvfWriter[] writers = new IvfWriter[numLayers];
        FileOutputStream[] yuvWriters = new FileOutputStream[numLayers];
        ...
        InputStream yuvStream;
        if (inputYuvFilename != null) {
            yuvStream = new FileInputStream(inputYuvFilename);
        } else {
            yuvStream = new FileInputStream(mInpPrefix + inputResource);
        }

        for (int i = 0; i < numLayers; i++) {
            String ivfFile = inputIvfFilename.replace(".ivf", "_layer" + i + ".ivf");
            String yuvFile = inputIvfFilename.replace(".ivf", "_layer" + i + ".yuv");
            outputFilenames.add(new Pair<>(ivfFile, yuvFile));
            writers[i] = new IvfWriter(ivfFile, mimeType, width, height);
            yuvWriters[i] = new FileOutputStream(yuvFile);
        }

        int[][] expectedTemporalId = {{0, 1, 0, 1}, {0, 2, 1, 2}};
        int temporalLayerCycle = 4;
        int frameIndex = 0;
        ...
        try {
            for (int i = 0; i < frameCount; i++) {
                ...
                // Read YUV frame
                int bytesRead = yuvStream.read(yuvFrame);
                if (bytesRead == -1) {
                    // unexpected EOF, looping
                    yuvStream.close();
                    if (inputYuvFilename != null) {
                        yuvStream = new FileInputStream(inputYuvFilename);
                    } else {
                    ...
                    bytesRead = yuvStream.read(yuvFrame);
                }

                if (bytesRead != frameSize) {
                    throw new RuntimeException(
                            "Failed to read full YUV frame at index "
                                    + i
                                    + ": "
                                    + bytesRead
                                    + " vs "
                ...
                int expectedId;
                if (temporalLayerIds != null
                        && i < temporalLayerIds.size()
                        && temporalLayerIds.get(i) >= 0) {
                    expectedId = temporalLayerIds.get(i);
                } else {
                    // Fallback to default temporal layer ids if we can't find it.
                    expectedId = expectedTemporalId[numLayers - 2][frameIndex % temporalLayerCycle];
                }

                // Write to all streams >= expectedId
                for (int layer = expectedId; layer < numLayers; layer++) {
                    writers[layer].writeFrame(frame, info.presentationTimeUs);
                    yuvWriters[layer].write(yuvFrame);
                }
            ...
            for (int i = 0; i < numLayers; i++) {
                ...
                if (yuvWriters[i] != null) yuvWriters[i].close();
            }
        }
