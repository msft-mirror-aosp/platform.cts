...
public final class CarPropertyConfigTest extends AbstractCarTestCase {
    ...
    public void testGetPropertyId() {
        List<Integer> expectedPropertyTypes =
                Arrays.asList(
    ...
    public void testGetAccess() {
        for (CarPropertyConfig<?> cfg : mConfigs) {
            ...
            for (AreaIdConfig<?> areaIdConfig : cfg.getAreaIdConfigs()) {
                ...
                if (areaIdAccess == CarPropertyConfig.VEHICLE_PROPERTY_ACCESS_READ) {
                    ...
                    throw new AssertionError(
                            "AreaIdConfig must have an access level of one of the"
            ...
            if (propertyIdAccess == CarPropertyConfig.VEHICLE_PROPERTY_ACCESS_READ) {
                ...
                throw new AssertionError(
                        "CarPropertyConfig must have an access level of one of the"
                                + " following: {READ, WRITE, READ_WRITE}");
            }
        }
    }
    ...
    public void testGetAreaType() {
        List<Integer> expectedAreaTypes =
                Arrays.asList(
        ...
        for (CarPropertyConfig cfg : mConfigs) {
            ...
            if (result == VehicleAreaType.VEHICLE_AREA_TYPE_GLOBAL) {
                ...
                Assert.fail(
                        "Failed for property: " + VehiclePropertyIds.toString(cfg.getPropertyId()));
            }
        }
    }
    ...
    public void testGetChangeMode() {
        List<Integer> expectedChangeModes =
                Arrays.asList(
            ...
            for (int areaId : areaIds) {
                ...
                assertWithMessage(
                                "Property ID: "
                                        ...
                                        + " must be found in AreaIdConfigs list")
                        .that(found)
                        .isTrue();
            }
        }
            ...
            for (AreaIdConfig<?> areaIdConfig : areaIdConfigs) {
                ...
                assertWithMessage(
                                "Property ID: "
                                        + VehiclePropertyIds.toString(cfg.getPropertyId())
                                        + " area ID: 0x"
                                        + Integer.toHexString(areaIdConfig.getAreaId())
                                        + " min/max must be both defined or both null")
                        .that(minMaxCorrectlyDefined)
                        .isTrue();
                if (cfg.getPropertyType().equals(Integer.class)) {
                    if (areaIdConfig.getMinValue() != null) {
                        assertThat((Integer) areaIdConfig.getMaxValue())
                                .isAtLeast((Integer) areaIdConfig.getMinValue());
                    ...
                    if (areaIdConfig.getMinValue() != null) {
                        assertThat((Long) areaIdConfig.getMaxValue())
                                .isAtLeast((Long) areaIdConfig.getMinValue());
                    ...
                    if (areaIdConfig.getMinValue() != null) {
                        assertThat((Float) areaIdConfig.getMaxValue())
                                .isAtLeast((Float) areaIdConfig.getMinValue());
                ...
                assertThat(areaIdConfig.getAccess())
                        .isIn(
                                Arrays.asList(
    ...
    /**
     * The property value must be independently controllable in any two different AreaIDs in the
     * array. An area must only appear once in the array of AreaIDs. That is, an area must only be
     * part of a single AreaID in the array.
     *
     * @param areaIds
     * @return
     */
