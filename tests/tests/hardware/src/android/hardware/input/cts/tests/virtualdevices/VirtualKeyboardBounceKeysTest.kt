...
class VirtualKeyboardBounceKeysTest : VirtualDeviceSettingTestCase() {

    @get:Rule val checkFlagRule = DeviceFlagsValueProvider.createCheckFlagsRule()

    private val context = getInstrumentation().targetContext
    private var existingBounceKeysThreshold: Int = 0
    ...
    override fun onSetupSetting() {
        ...
        mRule.runWithTemporaryPermission({
            existingBounceKeysThreshold = InputSettings.getAccessibilityBounceKeysThreshold(context)
            InputSettings.setAccessibilityBounceKeysThreshold(context, BOUNCE_KEYS_THRESHOLD)
    ...
    override fun onTearDownSetting() {
        mRule.runWithTemporaryPermission({
            InputSettings.setAccessibilityBounceKeysThreshold(context, existingBounceKeysThreshold)
        }, Manifest.permission.INTERACT_ACROSS_USERS_FULL)
    }
    ...
    companion object {
        ...
        private const val BOUNCE_KEYS_THRESHOLD = 5000
    }
}
