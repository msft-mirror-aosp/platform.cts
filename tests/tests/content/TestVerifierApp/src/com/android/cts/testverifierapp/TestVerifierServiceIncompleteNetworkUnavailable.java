...
public class TestVerifierServiceIncompleteNetworkUnavailable extends TestVerifierService {
    @Override
    public void onVerificationRequired(VerificationSession session) {
        Log.i(TAG, "onVerificationRequired: " + session.getId());
        // Report incomplete with limited network as reason
        session.reportVerificationIncomplete(VERIFICATION_INCOMPLETE_NETWORK_UNAVAILABLE);
