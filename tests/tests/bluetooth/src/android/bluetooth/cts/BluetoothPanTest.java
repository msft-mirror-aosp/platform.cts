...
public class BluetoothPanTest {
    ...
    public void setUp() {
        ...
        assertThat(mAdapter.getProfileProxy(mContext, mListener, BluetoothProfile.PAN)).isTrue();
        ...
        verify(mListener, timeout(PROXY_CONNECTION_TIMEOUT.toMillis()))
                .onServiceConnected(eq(BluetoothProfile.PAN), captor.capture());
        mService = (BluetoothPan) captor.getValue();
    ...
    public void tearDown() {
        mAdapter.closeProfileProxy(BluetoothProfile.PAN, mService);
    ...
    public void closeProfileProxy() {
        mAdapter.closeProfileProxy(BluetoothProfile.PAN, mService);
        verify(mListener, timeout(PROXY_CONNECTION_TIMEOUT.toMillis()))
                .onServiceDisconnected(eq(BluetoothProfile.PAN));
    }
    ...
    public void setAndCheckBluetoothTethering() {
        ...
        assertThat(mService.isTetheringOn()).isFalse();
        mService.setBluetoothTethering(true);
        assertThat(mService.isTetheringOn()).isTrue();
        mService.setBluetoothTethering(false);
        assertThat(mService.isTetheringOn()).isFalse();
    }
}
