...
public class CVE_2024_34734 extends StsExtraBusinessLogicTestCase {
    ...
    public void testPocCVE_2024_34734() {
        try {
            ...
            // Register a broadcast receiver for catching exceptions from PocBackgroundService
            final CompletableFuture<Exception> PocBackgroundServiceException =
            ...
            context.registerReceiver(
                    new BroadcastReceiver() {
                        ...
                        public void onReceive(Context context, Intent intent) {
                            try {
                                PocBackgroundServiceException.complete(
                                        (Exception)
            ...
            try (AutoCloseable withLockScreenCloseable =
                ...
                // Launch PocBackgroundService and check for any exceptions
                context.startForegroundService(new Intent(context, PocBackgroundService.class));
                assume().that(PocBackgroundServiceException.get(mTimeout, TimeUnit.MILLISECONDS))
                        .isNull();

                // Wait for PocBackgroundService's notification
                final NotificationManager notificationManager =
                        context.getSystemService(NotificationManager.class);
                final String selfPackageName = context.getPackageName();
                assume().withMessage("PocBackgroundService's notification was not posted")
                        .that(
                                poll(
                                        () -> {
