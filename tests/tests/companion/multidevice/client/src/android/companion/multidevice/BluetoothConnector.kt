...
class BluetoothConnector(
    ...
    fun attachClientSocket(associationId: Int) {
        try {
            val device = getRemoteDevice(associationId)
    ...
    private fun getRemoteDevice(associationId: Int): BluetoothDevice {
        val association = cdm.myAssociations.stream()
                .filter { it.id == associationId }
                .findAny()
                .orElseThrow()
        val address = association.deviceMacAddress
        return adapter.getRemoteDevice(address.toString().uppercase())
    }
