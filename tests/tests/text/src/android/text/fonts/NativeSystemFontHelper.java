...
public class NativeSystemFontHelper {
    ...
    public static class FontDescriptor {
        ...
        public int hashCode() {
            return Objects.hash(
                    mFilePath, mWeight, mSlant, mIndex, Arrays.hashCode(mAxes), mLocale);
        }

        /** Compares two Locale objects for equality after canonicalizing them. */
        ...
        /** Compares two LocaleList objects for equality using canonicalized locales. */
            ...
            for (long fontPtr = nNext(iterPtr); fontPtr != 0; fontPtr = nNext(iterPtr)) {
                try {
                    ...
                    for (int i = 0; i < font.mAxes.length; ++i) {
                        font.mAxes[i] =
                                new FontVariationAxis(
                                        tagToStr(nGetAxisTag(fontPtr, i)),
                                        nGetAxisValue(fontPtr, i));
                    }
    ...
    /** Finds the best-matching system font for a given style and text. */
    public static Pair<File, Integer> matchFamilyStyleCharacter(
            String familyName,
            int weight,
            boolean italic,
            String languageTags,
        ...
        final long fontPtr =
                nMatchFamilyStyleCharacter(
                        familyName, weight, italic, languageTags, familyVariant, text);
        final int runLength =
    ...
    private static native long nMatchFamilyStyleCharacter(
            String familyName,
            int weight,
            boolean italic,
            String languageTags,
    ...
    private static native int nMatchFamilyStyleCharacter_runLength(
            String familyName,
            int weight,
            boolean italic,
            String languageTags,
