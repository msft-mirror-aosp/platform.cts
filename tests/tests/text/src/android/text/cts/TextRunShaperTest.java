...
public class TextRunShaperTest {
    ...
    public void shapeText_differentPaint() {
        ...
        PositionedGlyphs result1 =
                TextRunShaper.shapeTextRun(
                        text, 0, text.length(), 0, text.length(), 0f, 0f, false, paint);

        paint.setTextSize(50f); // Shape text with 50px
        PositionedGlyphs result2 =
                TextRunShaper.shapeTextRun(
                        text, 0, text.length(), 0, text.length(), 0f, 0f, false, paint);
    ...
    /**
     * Asserts that drawing a text run with {@link Canvas#drawTextRun} and drawing the shaped glyphs
     * with {@link Canvas#drawGlyphs} produce the identical bitmap.
     */
