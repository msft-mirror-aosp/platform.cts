...
/**
 * Tests for the TTFF (time to the first fix) validating whether TTFF is below the expected
 * thresholds in different scenario.
 */
public class GnssTtffTests extends GnssTestCase {
    ...
    /**
     * Test the TTFF in the case where there is a network connection for both cold and hot start
     * TTFF cases.
     *
     * <p>We first test the "COLD" start where different TTFF thresholds are chosen based on network
    ...
    /**
     * Test Scenario 1 Check whether TTFF is below the threshold on the cold start with Wifi ON 1)
     * Delete the aiding data. 2) Get GPS, check the TTFF value
     *
    ...
    private void checkTtffColdWithWifiOn(long threshold) throws Exception {
        ...
        checkTtffByThreshold(
                "checkTtffColdWithWifiOn", TimeUnit.SECONDS.toMillis(threshold), softAssert);
        softAssert.assertAll();
    }

    /**
     * Test Scenario 2 Check whether TTFF is below the threhold on the hot start with wifi ON
    ...
    private void checkTtffHotWithWifiOn(long threshold) throws Exception {
        SoftAssert softAssert = new SoftAssert(TAG);
        checkTtffByThreshold(
                "checkTtffHotWithWifiOn", TimeUnit.SECONDS.toMillis(threshold), softAssert);
        softAssert.assertAll();
    }
    ...
    private void ensureNetworkStatus() {
        assertTrue(
                "Device has to connect to Wifi or Cellular to complete this test.",
    ...
    private boolean hasCellularData() {
        ...
        NetworkInfo cellularNetworkInfo =
     ...
     * @param threshold, the threshold for the TTFF value
     */
    private void checkTtffByThreshold(String testName, long threshold, SoftAssert softAssert)
            throws Exception {
        ...
        softAssert.assertTrue(
                "Test case: "
                        + testName
                        + ", TTFF should be less than "
                        + threshold
                        + " . In current test, TTFF value is: "
                        + ttffTimeMillis,
                ttffTimeMillis < threshold);
    }
}
