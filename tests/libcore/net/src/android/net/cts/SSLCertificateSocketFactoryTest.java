...
public class SSLCertificateSocketFactoryTest {
    ...
    /**
     * Tests hostname verification for {@link SSLCertificateSocketFactory#createSocket(String,
     * int)}.
     *
     ...
     *
     * <p>{@link SSLCertificateSocketFactory} is documented to verify hostnames using the {@link
     * HostnameVerifier} returned by {@link HttpsURLConnection#getDefaultHostnameVerifier}, so this
     * test connects twice, once with the system default {@link HostnameVerifier} which is expected
     * to succeed, and once after installing a {@link NegativeHostnameVerifier} which will cause
     ...
     * other TLS failure modes and the default HostnameVerifier are tested elsewhere, see {@link
     * com.squareup.okhttp.internal.tls.HostnameVerifierTest} and
     ...
     *   <li>{@code createSocket()} verifies the remote hostname is correct using {@link
    ...
    /**
     * Tests hostname verification for {@link SSLCertificateSocketFactory#createSocket(Socket,
     * String, int, boolean)}.
     *
     * <p>This method should return a socket which is fully connected (i.e. TLS handshake complete)
     * and whose peer TLS certificate has been verified to have the correct hostname.
     *
     * <p>The TLS socket returned is wrapped around the plain socket passed into {@code
     * createSocket()}.
     *
    ...
    /**
     * Tests hostname verification for {@link SSLCertificateSocketFactory#createSocket(String, int,
     * InetAddress, int)}.
     *
     * <p>This method should return a socket which is fully connected (i.e. TLS handshake complete)
     * and whose peer TLS certificate has been verified to have the correct hostname.
    ...
    /**
     * Tests hostname verification for {@link SSLCertificateSocketFactory#createSocket(InetAddress,
     * int)}.
     *
     ...
     *
     * <p>Behaviour is tested by installing a {@link NegativeHostnameVerifier} and by calling {@link
     * #assertConnectedSocket} to ensure TLS handshaking but no hostname verification takes place.
     * Next, {@link #verifyHostname} is called to ensure that hostname verification is using the
     * {@link HostnameVerifier} returned by {@link HttpsURLConnection#getDefaultHostnameVerifier} as
     * documented.
     *
     * <p>Tests the following behaviour:-
     *
     ...
     *   <li>Calling {@link #verifyHostname} on the returned socket throws {@link
     *       SSLPeerUnverifiedException} if the remote hostname is invalid
     * </ul>
     */
    @Test
     ...
     *
     * <p>For the variants of createSocket which verify the remote hostname, {@code socket} should
     * already be fully connected.
     *
    ...
    /**
     * A HostnameVerifier which always returns false to simulate a server returning a certificate
