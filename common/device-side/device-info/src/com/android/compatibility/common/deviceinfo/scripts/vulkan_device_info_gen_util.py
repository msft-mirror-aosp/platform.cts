...
def generate_vk_version_dependent_structs():
    all_vk_versions = sorted(VK.VULKAN_VERSIONS_AND_STRUCTS_MAPPING.keys())

    vk_dependent_structs = []

    for vk_version in all_vk_versions[1:]:
        vk_dependent_structs.append(INDENT * 4 + "if (properties.getLong(KEY_API_VERSION) >= VK_API_VERSION_" + vk_version + ") {")
        vk_dependent_structs.append(generate_vk_dependent_groups(vk_version))
        if(vk_version.endswith("1_1")):
            vk_dependent_structs.append(vk_1_1_arrays)
        vk_dependent_structs.append(INDENT * 4 + "}")

    return "\n".join(vk_dependent_structs)
