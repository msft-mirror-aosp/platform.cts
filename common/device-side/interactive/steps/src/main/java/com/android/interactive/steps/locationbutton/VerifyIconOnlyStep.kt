...
/**
 * A manual verification step that asks the user to verify the location button is rendered as an
 * icon-only.
 */
@NotFullyAutomated(
    reason =
...
class VerifyIconOnlyStep :
    YesNoStep(
        "Does the location button display ONLY an icon, with no accompanying text?"
    )
