...
/**
 * A manual verification step that asks the user to verify the color configurations of the location
 * button.
 */
@NotFullyAutomated(
    reason =
...
class VerifyColorsStep :
    YesNoStep(
        "Verify the location button has a BLUE background, " +
            "WHITE text, WHITE icon, and BLACK stroke. Do the colors match exactly?"
    )
