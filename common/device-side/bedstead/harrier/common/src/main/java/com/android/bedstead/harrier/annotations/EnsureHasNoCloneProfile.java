...
/**
 * Mark that a test method should run on a user which has no Clone profile.
 *
 * <p>Your test configuration may be configured so that this test is only run on a user which has
 * no Clone profile. Otherwise, you can use {@code Devicestate} to ensure that the device enters
 * the correct state for the method.
 */
@Target({ElementType.METHOD, ElementType.ANNOTATION_TYPE, ElementType.TYPE})
@Retention(RetentionPolicy.RUNTIME)
@EnsureHasNoProfileAnnotation("android.os.usertype.profile.CLONE")
@UsesAnnotationExecutor(UsesAnnotationExecutor.MULTI_USER)
public @interface EnsureHasNoCloneProfile {
    /** Which user type the Clone profile should not be attached to. */
