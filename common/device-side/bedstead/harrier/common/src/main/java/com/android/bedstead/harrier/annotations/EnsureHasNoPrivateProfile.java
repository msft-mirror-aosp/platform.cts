...
/**
 * Mark that a test method should run on a user which has no private profile.
 *
 * <p>Your test configuration may be configured so that this test is only run on a user which has
 * no private profile. Otherwise, you can use {@code Devicestate} to ensure that the device enters
...
public @interface EnsureHasNoPrivateProfile {
     ...
     *
     * <p>Annotations with a lower priority will be resolved before annotations with a higher priority.
     *
