...
/** Parameterize a test so that it runs on the same user as the device admin. */
@Target({ElementType.METHOD, ElementType.TYPE})
@Retention(RetentionPolicy.RUNTIME)
@ParameterizedAnnotation(scope = ENTERPRISE)
@RequireRunOnInitialUser
@EnsureHasNoDpc
@EnsureHasDeviceAdmin(isPrimary = true)
public @interface IncludeRunOnDeviceAdminUser {
    /**
