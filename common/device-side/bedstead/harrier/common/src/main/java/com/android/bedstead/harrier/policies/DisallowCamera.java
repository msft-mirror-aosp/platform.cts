...
@EnterprisePolicy(dpc = {
        ...
        APPLIED_BY_PROFILE_OWNER_PROFILE  | APPLIED_BY_PARENT_INSTANCE_OF_ORGANIZATIONAL_OWNED_PROFILE_OWNER_PROFILE | APPLIES_TO_OWN_USER
                | CANNOT_BE_APPLIED_BY_ROLE_HOLDER | INHERITABLE})
// The tests need to be split because the behaviour when using the permission is different (you can't use setCameraDisabled using permission)
//permissions = @EnterprisePolicy.Permission(appliedWith = MANAGE_DEVICE_POLICY_CAMERA, appliesTo = APPLIES_TO_OWN_USER))
public final class DisallowCamera {
}
