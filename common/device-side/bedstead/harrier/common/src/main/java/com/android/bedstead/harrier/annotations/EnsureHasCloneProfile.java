...
/**
 * Mark that a test method should run on a user which has a Clone profile.
 *
 * <p>Your test configuration may be configured so that this test is only run on a user which has
 * a Clone profile. Otherwise, you can use {@code Devicestate} to ensure that the device enters
...
public @interface EnsureHasCloneProfile {
    ...
    /** Whether the instrumented test app should be installed in the Clone profile. */
