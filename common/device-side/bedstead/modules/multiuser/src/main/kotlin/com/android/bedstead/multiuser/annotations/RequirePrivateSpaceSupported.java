...
import static com.android.bedstead.multiuser.annotations.EnsureHasPrivateProfile.ENSURE_HAS_PRIVATE_PROFILE_PRECEDENCE;
...
/**
 * Mark that a test method should run only if private space is supported on the device.
 *
 * <p>Your test configuration may be configured so that this test is only run on a user which
 * supports private space. The annotation uses {@code android.os.UserManager#canAddPrivateProfile()}
 * to check if private space is supported.
 */
...
public @interface RequirePrivateSpaceSupported {
     ...
     * {@link int}.
     */
    int priority() default ENSURE_HAS_PRIVATE_PROFILE_PRECEDENCE - 1;
}
