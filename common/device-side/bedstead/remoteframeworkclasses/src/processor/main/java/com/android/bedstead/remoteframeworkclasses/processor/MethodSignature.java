...
import static com.android.bedstead.remoteframeworkclasses.processor.Processor.CLASSES_LISTED_IN_TEST_CURRENT_FILE;
...
public final class MethodSignature {
     ...
     *
     * <p>Since such classes are not available on environments where test sdk is disabled. We
     * identify them so that we can replace them with proxy classes generated by the
     * TestApisReflection module.
     */
    private static boolean isTestClass(String typeName, Elements elements) {
        boolean isListedInTestCurrentFile = CLASSES_LISTED_IN_TEST_CURRENT_FILE.stream()
                        .anyMatch(c -> c.getName().equals(typeName));
        // wouldn't be accessible when test sdk is disabled.
        boolean isNotAccessible = elements.getTypeElement(typeName) == null;
