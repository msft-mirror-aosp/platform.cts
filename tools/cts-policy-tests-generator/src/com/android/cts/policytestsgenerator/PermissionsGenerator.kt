...
// Returns the permissions that can be used to set the policy.
object PermissionsGenerator {
    fun generateForUserScope(metadata: PolicyMetadata): List<String> {
        ...
        return listOf(metadata.requiredPermission).filter { it.isNotEmpty() }
    }
    ...
    fun generateForDeviceScope(metadata: PolicyMetadata): List<String> {
        ...
        return listOf(metadata.requiredPermission, metadata.requiredCrossUserPermission).filter {
            it.isNotEmpty()
        }
    }
